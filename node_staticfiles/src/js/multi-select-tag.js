export default function MultiSelectTag(e, t={
    shadow: !1,
    rounded: !0
}) {
    var n = null
    ...
    function() {
        ...
        // custom
        spanHelperText = document.createElement("span"); spanHelperText.classList.add("span-helper-text"); spanHelperText.textContent = t.helperText || "click in button";
        const arrowParser = new DOMParser;
        const arrow = arrowParser.parseFromString('<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" /> </svg>', "image/svg+xml").documentElement;
        spanHelperText.append(arrow);
        o.insertBefore(spanHelperText, o.firstChild);
        // end custom
    ...
    // custom
    function toggleHelperTextVisibility() {
        if (n.selectedIndex >= 0) {spanHelperText.classList.add("hidden")}
        else {spanHelperText.classList.remove("hidden")}
    }
    // endcustom
} 
